...
<head>
    <meta charset="UTF-8">
    <title>credential_manager API Documentation</title>
...
func registerBmcCredentialPaths(spec *openapi3.T) {
	...
	statusSchema, _ := openapi3gen.NewSchemaRefForValue(&v1.BmcCredentialStatus{}, spec.Components.Schemas)
	spec.Components.Schemas["BmcCredentialStatus"] = statusSchema
	...
	// Patch BmcCredential operation
	patchOp := openapi3.NewOperation()
	patchOp.OperationID = "patchBmcCredential"
	patchOp.Summary = "Patch a BmcCredential resource"
	patchOp.Description = "Partially updates an existing BmcCredential resource using patch semantics"
	patchOp.Tags = []string{"BmcCredential"}
	...
	// Update BmcCredential status operation
	updateStatusOp := openapi3.NewOperation()
	updateStatusOp.OperationID = "updateBmcCredentialStatus"
	updateStatusOp.Summary = "Update BmcCredential status"
	updateStatusOp.Description = "Updates only the status subresource for an existing BmcCredential"
	updateStatusOp.Tags = []string{"BmcCredential"}
	updateStatusOp.RequestBody = &openapi3.RequestBodyRef{
		Value: openapi3.NewRequestBody().
			WithRequired(true).
			WithJSONSchemaRef(&openapi3.SchemaRef{Ref: "#/components/schemas/BmcCredentialStatus"}),
	}
	...
	updateStatusOp.Responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			...
			WithJSONSchemaRef(&openapi3.SchemaRef{Ref: "#/components/schemas/BmcCredential"}),
	})
	...
	// Patch BmcCredential status operation
	patchStatusOp := openapi3.NewOperation()
	patchStatusOp.OperationID = "patchBmcCredentialStatus"
	patchStatusOp.Summary = "Patch BmcCredential status"
	patchStatusOp.Description = "Partially updates only the status subresource for an existing BmcCredential"
	patchStatusOp.Tags = []string{"BmcCredential"}
	...
	spec.Paths.Set("/bmccredentials/{uid}/status", statusPath)
}
