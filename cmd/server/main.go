...
import (
	...
	_ "github.com/user/credential-manager/pkg/apiversion"
	...
	"github.com/user/credential-manager/internal/storage/ent/migrate"

	_ "github.com/mattn/go-sqlite3"

	"github.com/openchami/fabrica/pkg/events"
	. "github.com/user/credential-manager/internal/middleware"

	"github.com/openchami/fabrica/pkg/reconcile"
	"github.com/user/credential-manager/pkg/reconcilers"
)
...
func DefaultConfig() *Config {
	return &Config{
		...
		ReconcileWorkers: 5,
		MasterKey:        "",
		SecretsFile:      "/tmp/vault-secrets.json",
...
var rootCmd = &cobra.Command{
	...
	Short: "",
	Long:  `credential-manager - A Fabrica-generated OpenCHAMI service`,
	RunE:  runServer,
}

var serveCmd = &cobra.Command{
	Use:   "serve",
	Short: "Start the credential-manager server",
	Long:  `Start the credential-manager HTTP server with the configured options`,
	RunE:  runServer,
}
...
func init() {
	...
	// Global flags
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.credential-manager.yaml)")
	...
	serveCmd.Flags().String("master-key", "", "MASTER_KEY override (64-character hex string)")
	serveCmd.Flags().String("secrets-file", "/tmp/vault-secrets.json", "Path to local secrets file")
...
func initConfig() {
	...
	if cfgFile != "" {
		...
		viper.SetConfigName(".credential-manager")
	}

	// Environment variables
	viper.SetEnvPrefix("CREDENTIAL-MANAGER")
	viper.AutomaticEnv()
...
func runServer(cmd *cobra.Command, args []string) error {
	log.Printf("Starting credential-manager server...")
	...
	if masterKey == "" {
		return fmt.Errorf("MASTER_KEY is required and must be a 64-character hex string")
	}

	secretsFile := strings.TrimSpace(config.SecretsFile)
	if secretsFile == "" {
		secretsFile = strings.TrimSpace(os.Getenv("SECRETS_FILE"))
	}
	if secretsFile == "" {
		secretsFile = "/tmp/vault-secrets.json"
	}

	localStore, err := secrets.NewLocalSecretStore(masterKey, secretsFile, true)
	if err != nil {
		return fmt.Errorf("failed initializing local secret store: %w", err)
	}
	log.Printf("Local secret store initialized: %s", secretsFile)
	...
	if eventConfig.EventTypePrefix == "" {
		eventConfig.EventTypePrefix = "credential-manager.resource"
	}
	...
	if config.ReconcileEnabled {
		...
		if err := reconcilers.RegisterReconcilers(controller, storageClient, eventBus, localStore, insecureHTTPClient); err != nil {
...
func healthHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte(`{"status":"healthy","service":"credential-manager"}`))
}
...
var versionCmd = &cobra.Command{
	...
	Long:  `Print the version number of credential-manager`,
	Run: func(cmd *cobra.Command, args []string) {
		fmt.Println("credential-manager v1.0.0")
	},
}
