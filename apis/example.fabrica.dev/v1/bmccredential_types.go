...
type BmcCredentialSpec struct {
	...
	CurrentPasswordSecretID  string `json:"currentPasswordSecretID" validate:"required"`
	TargetAccount            string `json:"targetAccount" validate:"required"`
	NewPasswordSecretID      string `json:"newPasswordSecretID" validate:"required"`
}

// BmcCredentialStatus defines the observed state of BmcCredential
type BmcCredentialStatus struct {
	State                string    `json:"state,omitempty"`
	LastUpdatedAtUTC     time.Time `json:"lastUpdatedAtUTC,omitempty"`
